...
| Post-login Action (`onExecuteLogin`) | `api.accessToken.setCustomClaim('https://heka/roles', [role])`, `…/name` = `event.user.username ?? event.user.nickname`, `…/org_id`, `…/heka_uid` = `event.user.app_metadata.heka_uid ?? event.user.user_id`. Namespaced names are mandatory in access tokens with an API audience. |
| Credentials-exchange Action (`onExecuteCredentialsExchange`) | Same claims for the M2M client. |
