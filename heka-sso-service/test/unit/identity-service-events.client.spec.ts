 ...
 * (P2.2): bearer-authenticated connect, verifier-event filtering, reconnect
 * with backoff, token invalidation on an unauthorized close.
 */
describe('IdentityServiceEventsClient (P2.2)', () => {
  ...
  test('ignores non-verifier notifications and malformed frames', async () => {
    ...
    const received = await waitFor(() => events[0], 'forwarded event')
    expect(received).toEqual({ sessionId: 'vs-1', state: VerificationSessionState.Error, errorMessage: 'nonce mismatch' })
